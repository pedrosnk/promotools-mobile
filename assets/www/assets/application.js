...
window.utils = {
  ...
  showNextQuestion : function(next){    
    ...
    $(current).transition({
      ...
      complete: function(){
        ...
        utils.resetItemRateQuestionTpl();
      },
      opacity: 0 
    });
  ...
  finishSurvey : function(){
    setTimeout(function(){
      utils.showNextQuestion($("#thanks-message"));
    }, 1000);
    utils.submitSurvey();
    /*  
    setTimeout(function() {
  ...
  submitSurvey : function(){
    var surveys = [config.DATA_SURVEY];
  ...
  resetItemRateQuestionTpl : function() {
    $(".select-answer").removeClass("bad-face regular-face good-face awesome-face");
    $(".select-answer p").show();    
  }  
}
;
...
if (habibsFormView && habibsFormView.length > 0) {
  ...
  window.SurveyView = Backbone.View.extend({
    ...
    render : function() {
      ...
      this.surveyDataModel = new Survey( {
        url: '/v1.0/surveys/vintage/conjunto'
      });
